...
def create_clip():
    ...
    base_cmd = [
        ...
        '-vf', 'split[original][copy];[copy]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,gblur=sigma=50,boxblur=luma_radius=30:luma_power=3[blurred];[original]scale=1080:1920:force_original_aspect_ratio=decrease[scaled];[blurred][scaled]overlay=(W-w)/2:(H-h)/2:format=auto',
    ...
    if ap_config.get('enabled', True):
        ...
        vf_filters = 'split[original][copy];[copy]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,gblur=sigma=50,boxblur=luma_radius=30:luma_power=3[blurred];[original]scale=1080:1920:force_original_aspect_ratio=decrease[scaled];[blurred][scaled]overlay=(W-w)/2:(H-h)/2:format=auto'
